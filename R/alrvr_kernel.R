...
alrvr_kernel <- function(e, kmax = 0, kernel = "bartlett", criterion = "bic") {
    ...
    if (k == 0)
        ...
        temp <- cbind(res, lagn(res, 1))[2:nrow(res), , drop = FALSE]
        x_tmp <- temp[, 2, drop = FALSE]
        a <- solve(t(x_tmp) %*% x_tmp) %*% t(x_tmp) %*% temp[, 1, drop = FALSE]
            ...
            if (kernel == "bartlett")
                ...
                w <- 25 / (12 * pi^2 * (i / l)^2) *
                    (sin(6 * pi * i / (l * 5)) / (6 * pi * i / (l * 5)) -
                        cos(6 * pi * i / (l * 5)))
            lrv <- lrv + 2 * as.numeric(t(res[1:(nrow(res) - i)]) %*%
                                        res[(1 + i):nrow(res)]) * w / t
        }
    }
