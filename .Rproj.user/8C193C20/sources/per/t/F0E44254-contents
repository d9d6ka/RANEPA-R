...
##       detho = dekpss2(model,t,tb1,tb2); # nolint
## INPUT:
...
determi_kpss_2p <- function(model, t, tb1, tb2) {
    cns <- matrix(data = 1, nrow = t, ncol = 1)
    tend <- matrix(data = 1:t, nrow = t, ncol = 1)

    if (model == 1) {
        du1 <- rbind(
            matrix(data = 0, nrow = tb1, ncol = 1),
            matrix(data = 1, nrow = t - tb1, ncol = 1)
        )
        du2 <- rbind(
            matrix(data = 0, nrow = tb2, ncol = 1),
            matrix(data = 1, nrow = t - tb2, ncol = 1)
        )
        z <- cbind(cns, du1, du2)
    }
    else if (model == 2) {
        ...
        dt1 <- rbind(
            matrix(data = 0, nrow = tb1, ncol = 1),
            matrix(data = 1:(t - tb1), nrow = t - tb1, ncol = 1)
        )
        dt2 <- rbind(
            matrix(data = 0, nrow = tb2, ncol = 1),
            matrix(data = 1:(t - tb2), nrow = t - tb2, ncol = 1)
        )
